...
const CoverAboutus = () => {
  ...
  const handleChange = (e, index) => {
    ...
    const updatedText = [...showAboutus];
    updatedText[index] = { ...updatedText[index], text: value };
    setShowAboutus(updatedText);
  };
  const handleSubmit = async (aboutusId, text) => {
    const emptyFields = showAboutus.filter(
      (aboutus) => aboutus.text.trim() === ""
    );
    ...
    <div className="text-left flex items-center justify-center p-12">
      {showAboutus.map((aboutus, index) => (
        <div className="mx-auto w-full" key={aboutus._id}>
          <div className="bg-white overflow-hidden shadow rounded-lg border">
            <div className="px-4 py-5 sm:px-6">
              <div>
                ...
                >
                  Content of Cover Text <span className="text-red-500">*</span>
                </label>
          ...
          <button
            type="button"
            onClick={(e) => handleSubmit(aboutus._id, aboutus.text)}
