...
const History = () => {
  ...
  const handleChange = (e, index) => {
    ...
    const updatedHistory = [...showAboutus];
    updatedHistory[index] = { ...updatedHistory[index], historys: value };
    setShowAboutus(updatedHistory);
  };
  const handleSubmit = async (aboutusId, historys) => {
    const emptyFields = showAboutus.filter(
      (aboutus) => aboutus.historys.trim() === ""
    );
    ...
    <div className="text-left flex items-center justify-center p-12">
      {showAboutus.map((aboutus, index) => (
        <div className="mx-auto w-full" key={aboutus._id}>
          <div className="bg-white overflow-hidden shadow rounded-lg border">
            <div className="px-4 py-5 sm:px-6">
              <div>
                ...
                >
                  Content of History <span className="text-red-500">*</span>
                </label>
          ...
          <button
            type="button"
            onClick={(e) => handleSubmit(aboutus._id, aboutus.historys)}
